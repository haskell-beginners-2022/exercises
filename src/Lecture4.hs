...
parseRow l = do
  [prod, trade, cost] <- Just $ split l
  guard $ prod /= ""
  readMaybe trade >>= \t -> parseCost cost >>= \c -> pure $ Row prod t c
...
instance Semigroup MaxLen where
  (<>) MaxLen {unMaxLen = a} MaxLen {unMaxLen = b}
    | length a >= length b = MaxLen {unMaxLen = a}
    | otherwise = MaxLen {unMaxLen = b}
...
combineRows rs = sconcat $ rowToStats <$> rs
...
displayStats
  Stats
    { statsTotalPositions = Sum a,
      statsTotalSum = Sum b,
      statsAbsoluteMax = Max c,
      statsAbsoluteMin = Min d,
      statsSellMax = e,
      statsSellMin = f,
      statsBuyMax = g,
      statsBuyMin = h,
      statsLongest = MaxLen {unMaxLen = i}
    } =
    "Total positions        : "
      <> show a
      <> "\nTotal final balance    : "
      ...
      <> show d
      <> "\nMax earning            : "
      <> maybe "no value" (show . getMax) e
      <> "\nMin earning            : "
      <> maybe "no value" (show . getMin) f
      <> "\nMax spending           : "
      <> maybe "no value" (show . getMax) g
      <> "\nMin spending           : "
      <> maybe "no value" (show . getMin) h
...
calculateStats = maybe "empty file" (displayStats . combineRows) . nonEmpty . mapMaybe parseRow <$> lines

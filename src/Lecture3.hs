...
--  them first).
--
-- 🕯 HINT: Check 'Enm' and 'Bounded' typeclasses.
--
...
-- can not be created. There is no value for the first argument of
-- the constructor 'List1' that will satisfy the a <> mempty == a
-- constraint, considering mempty would have to have at least
-- one item.
--
...
instance (Semigroup a, Show a) => Monoid (Treasure a) where
  mempty = NoTreasure
...
-- is of kind *, Foldable needs kind * -> *

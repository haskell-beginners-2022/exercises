...
-- "ccaabb"
duplicate :: [a] -> [a]
duplicate list = concat (map (\x -> [x, x]) list)
...
dropSpaces = takeWhile (not . Char.isSpace) . dropWhile Char.isSpace
...
experiencePoints :: DragonKind -> Int
experiencePoints kind =
  case kind of
    RedDragon ->
      100
    BlackDragon ->
...
dragonReward :: DragonKind -> Chest a -> (Int, Maybe a)
dragonReward kind (Chest gold reward) =
  case kind of
    RedDragon ->
      (gold, Just reward)
    BlackDragon ->
...
dragonFight knight dragon = KnightRuns
...
eval vars expr =
  case expr of
    Lit val ->
      Right val
    Var val ->
      case lookup val vars of
        Just v -> Right v
        Nothing -> Left (VariableNotFound val)
    Add first second ->
      case (eval vars first, eval vars second) of
        (Right f, Right s) ->
          Right (f + s)
        (Left err, _) ->
          Left err
...
constantFolding expr =
  case expr of
    ...
    Add (Lit f) (Lit s) ->
      Lit (f + s)
    Add (Add (Lit x) val) (Lit y) ->
      constantFolding (Add (Lit (x + y)) (constantFolding val))
    Add (Add val (Lit x)) (Lit y) ->
      constantFolding (Add (Lit (x + y)) (constantFolding val))
    Add (Lit y) (Add (Lit x) val) ->
      constantFolding (Add (Lit (x + y)) (constantFolding val))
    Add (Lit y) (Add val (Lit x)) ->
      constantFolding (Add (Lit (x + y)) (constantFolding val))
    Add (Add (Lit x) val) (Add (Lit y) val2) ->
      constantFolding (Add (Lit (x + y)) (constantFolding (Add (constantFolding val) (constantFolding val2))))
    Add (Add (Lit x) val) (Add val2 (Lit y)) ->
      constantFolding (Add (Lit (x + y)) (constantFolding (Add (constantFolding val) (constantFolding val2))))
    Add (Add val (Lit x)) (Add (Lit y) val2) ->
      constantFolding (Add (Lit (x + y)) (constantFolding (Add (constantFolding val) (constantFolding val2))))
    Add (Add val (Lit x)) (Add val2 (Lit y)) ->
      constantFolding (Add (Lit (x + y)) (constantFolding (Add (constantFolding val) (constantFolding val2))))
    _ ->
      expr

...
subString start end str
    ...
    | otherwise  = take (end - start + 1) rest
        where rest = drop start str
...
lowerAndGreater n list = show n ++ " is greater than " ++ show gt ++ " elements and lower than " ++ show lt ++ " elements"
    where [gt, lt] = go 0 0 list
            where
                go:: Int -> Int -> [Int] -> [Int]
                go a b [] = [a, b]
                go a b (x:xs)

...
sumOfSquares :: Num a => a -> a -> a -- O Int -> Int -> Int
...
minmax x y z = maximo - minimo
    where maximo = maximum [x, y, z]
          minimo = minimum [x, y, z] 

minmax2 :: Int -> Int -> Int -> Int
minmax2 x y z = maximum lista - minimum lista 
    where lista = [x, y, z]
...
subString start end str
    ...
    | otherwise = take amountToTake (drop start str)
        where amountToTake = end - start + 1

subStringWithNoNegativeCheck :: Int -> Int -> String -> String
subStringWithNoNegativeCheck start end str = take amountToTake str 
        where amountToTake = end - start -- si start es negativo el resultado de la resta es más grande.
...
lowerAndGreater n list = show n ++ " is greater than " ++ show (greaterThan n list) ++ " elements and lower than " ++ show (lowerThan n list) ++ " elements"
    where 
        greaterThan n [] = 0
        greaterThan n (x:xs) 
            | n > x = 1 + greaterThan n xs 
            | otherwise = greaterThan n xs 
        lowerThan n [] = 0
        lowerThan n (x:xs)
            | n < x = 1 + lowerThan n xs 
            | otherwise = lowerThan n xs

...
minmax x y z
  | x > y && x < z || x < y && x > z = abs (y - z)
  | y < x && y > z || y > x && y < z = abs (x - z)
  | otherwise = abs (x - y)
...
lowerAndGreater n list = go 0 0 list
  where
    ...
    go g l list
      | null list = show n ++ " is greater than " ++ show g ++ " elements and lower than " ++ show l ++ " elements"
      | head list < n = go (g + 1) l (tail list)
      | head list > n = go g (l + 1) (tail list)
      | otherwise = go g l (tail list)

...
subString start end str = take (re - rs) $ drop rs str
    where 
        rs = max start 0
        re = max (end+1) 0
...
lowerAndGreater n list = show n ++ " is greater than " ++ show a ++ " elements and lower than " ++ show b ++ " elements"
    where 
        (a,b) = countLG list (0,0)
        countLG :: [Int] -> (Int, Int) -> (Int, Int)
        countLG [] counts = counts
        countLG (x:xs) (l,g)
                | x > n = countLG xs (l,g+1)
                | x < n = countLG xs (l+1,g)
                | otherwise = countLG xs (l,g)

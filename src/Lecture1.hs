...
subString start end str
  | null str || end < 0 || start > end = ""
  | otherwise  = [str !! i | i <- [(max 0 start)..end], i < length str]
...
-}
strSum :: String -> Int
strSum str = sum [read s | s <- words str]
...
lowerAndGreater n list = helper list 0 0
  where
    helper [] lwr grt = show n ++ " is greater than " ++ show lwr ++ " elements and lower than " ++ show grt ++ " elements"
    helper (x:xs) lwr grt
      | x < n          = helper xs (lwr + 1) grt
      | x > n          = helper xs lwr (grt + 1)
      | otherwise      = helper xs lwr grt

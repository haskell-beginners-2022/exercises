...
subString start end str
    ...
    | end < 0   = ""
    | otherwise = (take end . drop start) str
...
🕯 HINT: Use recursion to implement this function. hint: no
-}
lowerAndGreater :: Int -> [Int] -> String
lowerAndGreater n list = show n ++ "Is greater than " ++ show greater ++ " elements and lower than " ++ show (length list - greater) ++ " elements."
    where
        greater = length $ filter (<n) list

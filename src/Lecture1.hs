...
subString start end str
    | end < 0 = []
    | end < start = []
    | otherwise = let x = getStart start in take (end-x+1) (drop x str)
    where 
        getStart :: Int -> Int
        getStart st
            | st < 0 = 0
...
-}
strSum :: [Char] -> Int
strSum str = accum 0 (words str)
    where
        accum :: Int -> [String] -> Int
        accum agg list =
            if null list 
            then agg 
            else accum (agg + read (head list)) (tail list)
...
lowerAndGreater n list = 
    let ng = getCount (< n) 0 list 
        nl = getCount (> n) 0 list
    in show n ++ " is greater than " ++ show ng ++ " elements and lower than " ++ show nl ++ " elements"
    where
        getCount :: (Int -> Bool) -> Int -> [Int] -> Int
        getCount check count nums
            | null nums = count
            | check (head nums) =  getCount check (count + 1) (tail nums)
            | otherwise = getCount check count (tail nums)

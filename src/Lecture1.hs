...
subString start end str = if end < 0 then "" else drop start (take (end + 1) str)
...
lowerAndGreater n list = if null list then "10 is greater than 0 elements and lower than 0 elements" else go 0 0 list
    where
        go :: Int -> Int -> [Int] -> String
        go count equal tempList
          | null tempList = show n  ++ " is greater than " ++ show count ++ " elements and lower than " ++ show (length list - count - equal) ++ " elements"
          | head tempList > n = go count equal (tail tempList)
          | head tempList == n = go count (equal + 1)  (tail tempList)
          | otherwise = go (count + 1) equal (tail tempList)

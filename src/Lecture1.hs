...
-}
makeSnippet :: Int -> String -> String {-get first n elements from syntetic string-}
...
minmax x y z = maximum cont - minimum cont
  where
    cont = [x, y, z]
...
lowerAndGreater n list = makeText n numsLessAndMore{-(numLess, numMore)-}
  where
    makeText :: Int -> (Int, Int) -> String
    makeText a (b, c) = unwords [
      show a
      , "is greater than"
      , show b

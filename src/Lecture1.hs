...
lowerAndGreater n list = show n ++ " is greater than " ++ show lw ++ " elements and lower than " ++ show gr ++ " elements"
    where
        gr = length $ filter ( > n) list
        lw = length $ filter ( < n) list  
